...
const getOrderReview = async (req, res) => {
  try {
    const review = await Review.findOne({ order: req.params.id }).populate("user service");
    ...
    res.status(500).json({ message: "Failed to fetch order review", error: error.message });
  }
};
